...
if ($OJ_LOGIN_MOD != "hustoj") {
    $err_str = $err_str . "系统不允许注册.\\n";
    $err_cnt++;
}

if ($len > 20) {
    $err_str = $err_str . "用户名过长!\\n";
    $err_cnt++;
} else if ($len < 3) {
    $err_str = $err_str . "用户名过短!\\n";
    $err_cnt++;
}
if (!is_valid_user_name($user_id)) {
    $err_str = $err_str . "用户名仅可包含字母和数字!\\n";
    $err_cnt++;
}
$nick = trim($_POST['nick']);
$len = strlen($nick);
if ($len > 100) {
    $err_str = $err_str . "昵称过长!\\n";
    $err_cnt++;
} else if ($len == 0) {
    $nick = $user_id;
}
