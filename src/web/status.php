...
for ($i = 0; $i < $rows_cnt; $i++) {
    ...
    switch ($row['result']) {
        case 4:
            ...
            break;
        case 5:
            $MSG_Tips = "答案基本正确，但是格式不对。";
            break;
        case 6:
            $MSG_Tips = "答案不对，仅仅通过样例数据的测试并不一定是正确答案，一定还有你没想到的地方，点击查看系统可能提供的对比信息。";
            break;
        case 7:
            $MSG_Tips = "运行超出时间限制，检查下是否有死循环，或者应该有更快的计算方法";
            break;
        case 8:
            $MSG_Tips = "超出内存限制，数据可能需要压缩，检查内存是否有泄露";
            break;
        case 9:
            $MSG_Tips = "输出超过限制，你的输出比正确答案长了两倍，一定是哪里弄错了";
            break;
        case 10:
            $MSG_Tips = "运行时错误，非法的内存访问，数组越界，指针漂移，调用禁用的系统函数。请点击后获得详细输出";
            break;
        case 11:
            $MSG_Tips = "编译错误，请点击后获得编译器的详细输出";
            break;
        default:$MSG_Tips = "";
    ...
    if (intval($row['result']) == 11 && ((isset($_SESSION[$OJ_NAME . '_' . 'user_id']) && $row['user_id'] == $_SESSION[$OJ_NAME . '_' . 'user_id']) || isset($_SESSION[$OJ_NAME . '_' . 'source_browser']))) {
        $view_status[$i][3] .= "<a href='ceinfo.php?sid=" . $row['solution_id'] . "' class='" . $judge_color[$row['result']] . "  noradius btn-sm'  title='$MSG_Tips'>" . $MSG_Compile_Error . "";
    ...
    // （如果是格式错误或者答案错误 并且开启错误比较 或者运行时错误或者运行完成 ）并且用户为提交用户或者代码查看者
        ...
        $view_status[$i][3] .= "<a href='reinfo.php?sid=" . $row['solution_id']
            . "' class='" . $judge_color[$row['result']] . "  noradius btn-sm' title='$MSG_Tips'>" . $judge_result[$row['result']] . "";
        if ($row['result'] != 4 && isset($row['pass_rate']) && $row['pass_rate'] > 0 && $row['pass_rate'] < .98) {
            $view_status[$i][3] .= (100 - $row['pass_rate'] * 100) . "%</a>";
        } else {
            $view_status[$i][3] .= "</a>";
        ...
        if (!$lock || $lock_time > $row['in_date'] || $row['user_id'] == $_SESSION[$OJ_NAME . '_' . 'user_id']) {
            if ($OJ_SIM && $row['sim'] > 80 && $row['sim_s_id'] != $row['s_id']) {
                $view_status[$i][3] .= "<span class='" . $judge_color[$row['result']] . "  noradius btn-sm'  title='$MSG_Tips'>*" . $judge_result[$row['result']] . "";
                if ($row['result'] != 4 && isset($row['pass_rate']) && $row['pass_rate'] > 0 && $row['pass_rate'] < .98) {
