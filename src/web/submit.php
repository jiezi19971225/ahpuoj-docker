...
if (isset($_POST['id'])) {
    ...
    if ($rows_cnt != 1) {
        if (!isset($_SESSION[$OJ_NAME . '_' . 'administrator'])) {
            $errors = "<h2>你无法提交，该竞赛&作业不在运行时间内！</h2>";
            require "template/" . $OJ_TEMPLATE . "/error.php";
            exit(0);
        }
    } else {
        ...
        if ($isprivate == 1 && !isset($_SESSION[$OJ_NAME . '_' . 'c' . $cid])) {
            ...
            if ($ccnt == 0 && !isset($_SESSION[$OJ_NAME . '_' . 'administrator'])) {
                $errors = "<h2>你无法提交，你没有参加该竞赛&作业的权限！</h2>";
                require "template/" . $OJ_TEMPLATE . "/error.php";
                exit(0);
            }
        }
...
if ((~$OJ_LANGMASK) & (1 << $language)) {
    ...
    if (!isset($pid)) {
        ...
        $team_id = 0;
        $sql = "SELECT team_id FROM `contest_team_user` WHERE `contest_id` = ? and `user_id` = ?";
        $result = pdo_query($sql, $cid, $user_id);
        if (count($result) > 0) {
            $row = $result[0];
            $team_id = $row['team_id'];
        }
        $sql = "INSERT INTO solution_contest(solution_id,problem_id,user_id,in_date,language,ip,code_length,contest_id,team_id,num,result) VALUES(?,?,?,NOW(),?,?,?,?,?,?,14)";
        $res = pdo_query($sql, $insert_id, $id, $user_id, $language, $ip, $len, $cid, $team_id, $pid);
    }
